...
const std::string NAME = "coinrun_aisc_gem";
...
std::vector<std::string> WALKING_ENEMIES_AISC_GEM = {"slimeBlock", "slimePurple", "slimeBlue", "slimeGreen", "mouse", "snail", "ladybug", "wormGreen", "wormPink"};
std::vector<std::string> PLAYER_THEME_COLORS_AISC_GEM = {"Beige", "Blue", "Green", "Pink", "Yellow"};
std::vector<std::string> GROUND_THEMES_AISC_GEM = {"Dirt", "Grass", "Planet", "Sand", "Snow", "Stone"};

const int NUM_GROUND_THEMES = (int)(GROUND_THEMES_AISC_GEM.size());

class CoinRunAISCGem : public BasicAbstractGame {
  public:
    ...
    void asset_for_type(int type, std::vector<std::string> &names) override {
        if (type == PLAYER) {
            for (const auto &color : PLAYER_THEME_COLORS_AISC_GEM) {
            ...
            for (const auto &enemy : WALKING_ENEMIES_AISC_GEM) {
            ...
            names.push_back("kenney/Items/gemBlue.png");
        } else if (type == WALL_TOP) {
            for (const auto &ground : GROUND_THEMES_AISC_GEM) {
...
REGISTER_GAME(NAME, CoinRunAISCGem);
