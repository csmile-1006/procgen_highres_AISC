...
const std::string NAME = "coinrun_gem";
...
std::vector<std::string> WALKING_ENEMIES_GEM = {"slimeBlock", "slimePurple", "slimeBlue", "slimeGreen", "mouse", "snail", "ladybug", "wormGreen", "wormPink"};
std::vector<std::string> PLAYER_THEME_COLORS_GEM = {"Beige", "Blue", "Green", "Pink", "Yellow"};
std::vector<std::string> GROUND_THEMES_GEM = {"Dirt", "Grass", "Planet", "Sand", "Snow", "Stone"};

const int GROUND_THEMES = (int)(GROUND_THEMES_GEM.size());

class CoinRunGem : public BasicAbstractGame {
  public:
    ...
    void asset_for_type(int type, std::vector<std::string> &names) override {
        if (type == PLAYER) {
            for (const auto &color : PLAYER_THEME_COLORS_GEM) {
            ...
            for (const auto &enemy : WALKING_ENEMIES_GEM) {
            ...
            for (const auto &ground : GROUND_THEMES_GEM) {
    ...
    void game_reset() override {
        ...
        if (options.distribution_mode == EasyMode) {
            ...
            wall_theme = rand_gen.randn(GROUND_THEMES);
        }
